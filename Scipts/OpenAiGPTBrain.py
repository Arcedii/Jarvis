...
class LLMConfig:
    ...
    "Если пользователь спрашивает про погоду или просит открыть прогноз (\"погода\", "
    "\"что на улице\", \"прогноз\", \"weather\", \"forecast\") — "
    "в конце ответа добавь тег <<COMMAND=погода>>.\n"
    "Если не нужна команда — не добавляй теги. Не отказывайся добавлять тег: "
    "он всего лишь запускает локальное действие (открытие страницы погоды)."
    )
